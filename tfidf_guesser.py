...
class TfidfGuesser(BaseGuesser):
    ...
    def train(self, training_data: QantaDatabase, limit: int=-1) -> None:
        ...
        self.tfidf_vectorizer = TfidfVectorizer(
            ngram_range=(1, 2), max_df=.5, binary = True
        ).fit(x_array)
        self.tfidf_matrix = self.tfidf_vectorizer.transform(x_array)

    #         self.tfidf_matrix = sbert_model.encode(x_array)
    ...
    def guess(self, questions: List[str], max_n_guesses: Optional[int]) -> List[List[Tuple[str, float]]]:
        ...
        #         representations = sbert_model.encode(questions)
    ...
    def guess_wrong(self, questions: List[str], n) -> List[List[Tuple[str, float]]]:
        representations = self.tfidf_vectorizer.transform(questions)
        # print('transform complete')
        guess_matrix = self.tfidf_matrix.dot(representations.T).T
        # print('dot product complete')
        guess_indices = (-guess_matrix).toarray().argsort(axis=1)
        # print('guess complete')
        guesses = []
        for i in range(len(questions)):
            idxs = guess_indices[i]
            ...
            guesses_i = []
            for j in idxs:
                if self.i_to_ans[j] not in prev_answers:
                    prev_answers.add(self.i_to_ans[j])
                    cnt += 1
                    guesses_i.append((self.i_to_ans[j], guess_matrix[i, j]))
                    if cnt == n:
                        guesses.append(guesses_i)
                        break
    ...
    def confusion_matrix(self, evaluation_data: QantaDatabase, limit=-1) -> Dict[str, Dict[str, int]]:
        ...
        questions = [x.text for x in evaluation_data.dev_questions]
        answers = [x.page for x in evaluation_data.dev_questions]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--guesstrain", default="../custom_data/qanta.train.2018.json", type=str)
    parser.add_argument("--guessdev", default="../custom_data/qanta.dev.2018.json", type=str)
